...
class QrDetect():
    ...
    def get_rectangles_coords(self) -> Tuple[Tuple[Tuple[int, int], Tuple[int, int]]]:
        ...
        '''
        return tuple(map(lambda code: ((code.rect.left, code.rect.top),
                                       (code.rect.left + code.rect.width,
                                        code.rect.top + code.rect.height)), self.codes))
    ...
    def get_polygons(self) -> np.array:
        ...
        return np.array(tuple(map(lambda c: tuple(map(lambda p: (p.x, p.y), c.polygon)), self.codes)), np.int32)
    ...
    def get_texts(self) -> Tuple[Tuple[str, Tuple[int, int]]]:
        ...
        #return codes
        return tuple(map(lambda c: (f'({c.type}): {c.data.decode("utf-8")}', (c.rect.left, c.rect.top)), self.codes))