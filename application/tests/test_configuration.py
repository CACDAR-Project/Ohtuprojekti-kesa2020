...
class InitializerTest(unittest.TestCase):
    ...
    def setUp(self):
        self.list_with_lines = [
            ...
            '#Comment = this_is',
            'another_var=12',
            'this_is_a bad_line and_next is empty',
            '',
            'bool-value=True',
            'whut=what=with'
        ]
        self.valid_lines_indices = (0, 2, 5)
        self.list_filtered = list(tuple(self.list_with_lines[i].split('=')) for i in self.valid_lines_indices)
    ...
    def test_filter_map_settings_return_list_with_str_tuples(self):
        ...
        self.assertTrue(isinstance(res[0], tuple), 'List does not contain tuples')
        self.assertEqual(len(res[0]), 2, 'Tuple should have 2 elements, variable and value')
        for val in res[0]:
            self.assertTrue(isinstance(val, str), 'Element in tuple is not of type str')
    ...
    def test_filter_map_settings_list_with_bad_lines(self):
        res = Initializer.filter_map_settings(self.list_with_lines, comment_sign = '#', assignment_sign = '!!')
...
class ConfigurationTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.none_instance = Configuration.get_instance() # Should be None before initialization
    ...
    def test_configuration_raises_exception_when_initializing_several_objects(self):
        with self.assertRaises(RuntimeError) as context_mgr:
            conf = Configuration(False)
        self.assertEqual(str(context_mgr.exception), 'Settings object already created, cannot instantiate a new one. Please use the existing one.')
